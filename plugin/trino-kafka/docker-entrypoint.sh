...
if [ -n "$TRINO_RANGER_CONFIG_PATH" ]; then
    TRINO_RANGER_CONFIG_PATH="${TRINO_RANGER_CONFIG_PATH%/}"
    cat "${TRINO_RANGER_CONFIG_PATH}/access-control.properties" > "$TRINO_CONFIG_PATH/access-control.properties"
    cat "${TRINO_RANGER_CONFIG_PATH}/ranger-trino-security.xml" > "$TRINO_CONFIG_PATH/ranger-trino-security.xml"
    cat "${TRINO_RANGER_CONFIG_PATH}/ranger-trino-audit.xml" > "$TRINO_CONFIG_PATH/ranger-trino-audit.xml"
    cp -a "${TRINO_RANGER_CONFIG_PATH}/." "/usr/lib/trino/plugin/apache-ranger/conf/"
fi
